...
public class GlideImageLoader {
    ...
    public void load(final String str, RequestOptions requestOptions) {
        if (str != null && requestOptions != null) {
            onConnecting();
            ProgressAppGlideModule.expect(str, new ProgressAppGlideModule.UIonProgressListener() { // from class: com.chettapps.invitationmaker.photomaker.utility.GlideImageLoader.1
            ...
            GlideApp.with(this.mImageView.getContext()).load(str).transition(DrawableTransitionOptions.withCrossFade()).thumbnail(0.1f).apply((BaseRequestOptions<?>) requestOptions.skipMemoryCache(true)).listener(new RequestListener<Drawable>() { // from class: com.chettapps.invitationmaker.photomaker.utility.GlideImageLoader.2
